 ...
 *
 * 注意那三条**不是**从随机池里摘出去的：没有 verdictExcuse 时，
 * 随机路径是从全部八条里挑（见 pickConfession 第 3 条规则）。
 * 它们只是额外多了一条"被认出来"的路。
 *
 * ── 语气 ────────────────────────────────────────────────────
...
export const CONFESSION_GROUPS = [
  ...
  },
  {
    ...
    entries: [
      ...
      {
        id: 'sixty_wan_limit',
        quote: '你说你还是有一些敬畏心的。你给自己定了一个六十万的限额，' +
          '只收六十万以下，不收六十万以上。你说金额大了不安全，' +
          '你也不希望有那种好像被卖给别人一样的感觉。',
        rebuttal: '法院最终认定，贪欲没有底线，六十万和六百万之间，只隔了一次侥幸。'
      }
    ]
  },
  {
 ...
 *
 * ── 为什么是 0.25 而不是 0.4 ────────────────────────────────
 * 这个数是**按十五条的池子反推的**：赤峰对夹只是其中一条，所以它的
 * 出现率不取决于"留几成给别的派别"这一件事，天花板是 1/15 ≈ 6.7%——
 * 就算把这个数压到 0（完全均匀），也才 6.7%。
 * 0.25 的算式是 0.75 × 1/15 = 5.0%，正好落在"稀有但不至于
 * 一辈子见不到"上。定 0.4 只有 2.7%，定 0.5 只有 3.3%，都够不着。
 *
 * ⚠️ 池子后来缩到八条，而且**五类不再等大**（1/3/2/1/1）。
 * 挑法是"先均匀挑类、再类内均匀挑条"，所以单条的那几类被**放大**了，
 * 不是按八条均分：赤峰对夹现在 0.75 × 1/5 = 15%（原 5.0%）；
 * 哲学反思派只剩一条，叠上规则 2 之后独占 0.25 + 0.75 × 1/5 = 40%。
 * "稀有"这个效果已经不成立了——实测 20 次里出现 3 次。
 * 这里**没有**跟着调（调它属于动数值，要单独跑一版实测再定）。
 *
 * 代价是哲学反思派从顶档的必选变成少数，那一派接"你连马桶水箱里都是美金"
 ...
 *   2. 非法所得 > 500 万 → 二成五落哲学反思派，七成五落到第 3 条；
 *   3. 其余（风险触顶的留置：他压根没来得及解释）→ 五类里随机挑一类，
 *      再从该类里随机挑一条。类内条数不等，所以**每条不再是等概率**：
 *      单条的那三类各 1/5，两条的那两类各 1/10，三条的不知情派各 1/15。
 *
 ...
 *
 * 顶档的语录原先必是哲学反思派（金额过线即 100%），那一派当时那句
 * 接"你连马桶水箱里都是美金"落差正好。现在只有二成五是它，
 * 另外七成五会挑到别的派别——接上去未必还有那个落差，
